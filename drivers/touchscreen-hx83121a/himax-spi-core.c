...
static void himax_unlock(struct himax_ts_data *ts)
{
	 ...
	 * touch interrupt. Only re-arm while the panel is still enabled and
	 * the device is not being torn down; suspend/remove paths leave it
	 * unmasked until the next reinit re-enables it.
	 */
	if (ts->panel_enabled && !ts->shutting_down)
